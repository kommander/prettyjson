...
var renderToArray = function(data, options, indentation, depth) {
  if (isSerializable(data, false, options)) {
    return [Utils.indent(indentation) + addColorToData(data, options)];
  }

  if (depth > options.depth) {
    return [Utils.indent(indentation) + '[' + typeof data + ']'];
  }
  ...
  if (Array.isArray(data)) {
    ...
    data.forEach(function(element) {
      ...
      if (isSerializable(element, false, options)) {
        line += renderToArray(element, options, 0, depth + 1)[0];
  ...
  Object.getOwnPropertyNames(data).forEach(function(i) {
    ...
    if (isSerializable(data[i], false, options)) {
      ...
      key += renderToArray(data[i], options, nextIndentation, depth + 1)[0];
      output.push(key);
...
exports.render = function render(data, options, indentation) {
  ...
  options.depth = options.depth || Infinity;

  options.stringColor = options.stringColor || null;

  return renderToArray(data, options, indentation, 0).join('\n');
};
...
exports.renderString = function renderString(data, options, indentation) {
  ...
  try {
    parsedData = options && options.parse
      ? options.parse(data)
      : JSON.parse(data);
  } catch (e) {
